...
export const Dropdown = ({ devider, children }: TsDropdown) => {
	return (
		<>
			{/* Dropdown Menu Items */}
            <ul className={`${devider ? 'br' : '' } gap-y-3 relative`}>
              { children }
            </ul>
		</>
	)
...
export const DropdownItem = ({ title, children, expand }: TsDropdownItem) => {
  ...
  return (
    <>
      {/* Dropdown Items */}
      <li>
        <button type="button" onClick={toggleDropdown} className={`inline-flex justify-between items-center ${isExpand} p-4 hover:bg-gray-100 group`}>
          <motion.div initial={{scale: 0}} animate={{scale: 1}} exit={{scale:0}}><FolderIcon className="w-6 h-6 text-gray-400"/></motion.div>
        ...
        <ul className={`transition-all duration-800 will-change-auto border-l border-gray-200 ml-7 pl-3 overflow-y-hidden ${ dropdownOpen ? 'max-h-[800px] opacity-1' : 'max-h-0 opacity-0' }`}>
          { children }
        </ul>
      </li>
    </>

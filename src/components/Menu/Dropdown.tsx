...
export const Dropdown = ({ devider, children }: TsDropdown) => {
	return (
		<>
			{/* Dropdown Menu Items */}
      <ul className={`${devider ? 'br' : '' } px-3 gap-y-2 relative dark:[&>li>button]:text-slate-200 hover:[&>li>button]:bg-gray-100 dark:hover:[&>li>button]:bg-slate-700`}>
        { children }
      </ul>
		</>
	)
  ...
  return (
    <>
      ...
      <li>
        ...
        <ul className={`px-3 transition-all duration-800 will-change-auto border-l border-gray-200 dark:border-slate-700 ml-5 2xl:ml-7 pl-3 overflow-y-hidden ${ dropdownOpen ? 'max-h-[800px] opacity-1' : 'max-h-0 opacity-0' } dark:[&>li>a]:text-slate-200 dark:hover:[&>li>a]:bg-slate-700`}>
          { children }
        </ul>
      </li>
    </>

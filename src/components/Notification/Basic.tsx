...
export const Basic = ({ message, type="info" }: Basic) => {
  ...
  const [isNotificationHide, setisNotificationHide] = useState(false);
	...
	<>
		{/* Notif */}
		{!isNotificationHide ? 
    (<section id="notification" className={`${type} flex justify-between items-center z-10 h-[60px] text-black py-[10px] px-[25px] text-sm overflow-hidden`}>
      ...
      <button onClick={hendleNotification} className="p-2 rounded-full hover:bg-white dark:hover:bg-gray-800"><XIcon className="w-5"/></button>
    </section>) : ''
    }
    </>
	)
